...
dummies = np.zeros((len(business_tmp),len(categories)))
...
business_tmp['business_cat']=pd.Series(dummies_array, dtype=numpy.dtype("object"))
...
reviews = pd.merge(business_tmp,reviews_tmp, on='business_id')
userCategories = reviews['business_cat'].groupby(reviews['user_id']).agg({"business_cat": lambda x: list(x.sum())})
